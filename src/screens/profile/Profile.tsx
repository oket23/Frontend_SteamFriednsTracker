...
import StatCard from "@/screens/profile/components/StatCard.tsx";
import Panel from "@/screens/profile/components/Panel.tsx";
import Row from "@/screens/profile/components/Row.tsx";
import Skeleton from "@/screens/profile/components/Skeleton.tsx";
import CenterMessage from "@/screens/profile/components/CenterMessage.tsx";
...
const Profile = () => {
    ...
    const { data, isLoading, isError } = useAuthQuery(true);
    ...
    if (!data) {
        return (
            <CenterMessage
                title={t("profile.notLoadedTitle")}
                desc={t("profile.notLoadedDesc")}
            />
        );
    }
    ...
    return (
        <div className={cn("relative min-h-[calc(99.7vh-7rem)] bg-slate-950 text-slate-100")}>
            ...
            <div className="relative mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
                <div className={cn("overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] shadow-[0_20px_60px_-30px_rgba(0,0,0,0.75)] backdrop-blur-xl")}>
                            ...
                            <div className="flex flex-col gap-2 sm:items-end">
                                <div className="flex flex-wrap gap-2 sm:justify-end">
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            "cursor-pointer group relative inline-flex items-center justify-center gap-2 overflow-hidden",
                                            "rounded-full px-5 py-2.5 text-sm font-bold text-white",
                                            "bg-gradient-to-r from-red-600 to-rose-600",
                                            "shadow-lg shadow-red-900/20 ring-1 ring-white/15",
                                            "transition-all duration-300 hover:shadow-red-500/35 hover:scale-[1.02] active:scale-[0.98]",
                    ...
                    <div className="px-6 py-5 sm:px-8">
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                            <StatCard
                                label={t("profile.stats.trackerSince")}
                                value={formatDate(data.createdAtUtc)}
                            />
                            <StatCard
                                label={t("profile.stats.steamCreated")}
                                value={formatDate(data.steamAccountCreatedAtUtc)}
                            />
                            <StatCard
                                label={t("profile.stats.lastSeen")}
                                value={formatDate(data.lastSeenAtUtc)}
                            />
                        </div>

                        <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-2">
                            <Panel title={t("profile.summary.title")}>
                                <div className="space-y-2">
                                    <Row
                                        k={t("profile.summary.status")}
                                        v={stateLabel(data.personaState)}
                                    />
                                    <Row
